...
pacman::p_load(sf, sfdep, readxl, plyr, kableExtra, tmap, tidyverse)
```
...
Firstly, here are some requirements for our aspatial data: Columns of interest: - KODE KELURAHAN: sub-district code - KELURAHAN: sub-district - SASARAN: target - BELUM VAKSIN: not yet vaccinated - TOTAL VAKSIN DIBERIKAN: total vaccine administered
...
Visualising combined_jakarta in terms of vaccinated and total population:

```{r}
#| code-fold: true
#| code-summary: Show the code!
vaccinated = tm_shape(combined_jakarta)+
  tm_fill("Total_Vaccine_Administered") +
  tm_borders(alpha = 0.5) +
  tm_layout(main.title="Vaccinated Count")
...
tmap_arrange(vaccinated, total_population)
```
...
### Re-joining the geospatial and aspatial data frames + EDA
```{r}
...
Visualising combined_jakarta in terms of vaccinated and total population again:
...
vaccinated = tm_shape(combined_jakarta)+
  tm_fill("Total_Vaccine_Administered") +
  tm_borders(alpha = 0.5) +
  tm_layout(main.title="Updated Vaccinated Count")
...
vacc_rate <- vacc_jakarta %>%
  ...
  dplyr::summarise(`vacc_rate` = (Total_Vaccine_Administered/Total_Population)) %>%
