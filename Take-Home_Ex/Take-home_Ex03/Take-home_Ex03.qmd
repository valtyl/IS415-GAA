...
Comparing this to the GWR predictive model results in section 4.5.5.5, the points are closer to the diagonal line which means that is it a better model compared to the GWR predictive model. Hence it is also better than the OLS model.
...
see if need to do diff kernels, look at in class ex 9 notes
XXXXXXXXXX insert result

This result shows that XXXX neighbour points will be the optimal bandwidth to be used for the geographical random forest model.

Saving the bandwidth in an RDS file as the code takes some time to run:
```{r}
#| code-fold: true
#| code-summary: Show the code!
#| eval: false
...
An alternative method to determine the optimal bandwidth is that we can borrow the bandwidth from the GWR method in section 4.4.5.2 where bw = 119. 
...
----------- check if should eval be false or not ----------- 
...
## 4.5 Comparison of Performance Between Conventional OLS Method VS Geographical Weighted Method

compare all the graphs
compare all the RMSE, bigger is better
look into variable importance of gwRF adaptive

### 4.5.1 Conventional OLS Method

### 4.5.2 Geographical Weighted Method
