...
In this in-class exercise, you will gain hands-on experience on how to model geographical accessibility by using R's geospatial analysis packages.

### 10.1.1 Learning Outcome

By the end of this in-class exercise, you will be able:
...
Four data sets will be used in this in-class exercise, they are:
...
Notice that for the purpose of this in-class exercise, a constant value of 100 is used. In practice, actual demand of the hexagon and capacity of the eldercare centre should be used.

## 10.5 Apsatial Data Handling and Wrangling

### 10.5.1 Importing Distance Matrix

The code chunk below uses *read_csv()* of **readr** package to import `OD_Matrix.csv` into RStudio. The imported object is a tibble data.frame called `ODMatrix`.
...
The imported ODMatrix organised the distance matrix column-wise.

![](images/he10-1-01.png){fig-align="center"}

On the other hands, most of the modelling packages in R is expecting a matrix look similar to the figure below.

![](images/he10-2-01.png){fig-align="center"}
...
EucMatrix <- SpatialAcc::distance(hexagon_coord,
                                  ...
                                  type="euclidean")
```
type can be manhattan or euclidean
...
acc_Hansen <- data.frame(ac(hexagons$demand,
                            eldercare$capacity,
                            distmat_km, 
                            # d0 = 50, #this is threshold, meaning 50km. No threshold means will take everyone into consideration
                            power = 2, 
                            family = "Hansen"))
```

![](images/he10-3-01.png){fig-align="center"}

ac() will give a value hence need to use data.frame() to make the output into a dataframe
...
![](images/he10-4-01.png){fig-align="center"}
...
![](images/he10-5-01.png){fig-align="center"}
...
```
bbox() gives the maximum and minimum coordinates of the map area
...
hexagon_Hansen <- st_join(hexagon_Hansen, mpsz, 
                          join = st_intersects)
```
join the planning subzone area (mpsz) with the hexagon_Hansen layer. the polygon hexagon_Hansen (3 variables) + mpsz (16 variables) = 19 variables
...
ggplot(data=hexagon_Hansen, 
  ...
  geom_point(stat="summary", 
             ...
             size=2)
```
need to use a log transformation! bc the different regions have extreme values, and it will cause the plot to look very flat and not much comparison we can analyse without doing the log transformation
